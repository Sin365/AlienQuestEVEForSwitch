		...
		else
		{
			...
			if (num < 1.2f)
			{
				...
				if (Active_Heal && Heal_Timer < 2f)
				{
					...
					{
						ActiveGlow_Timer = 0f;
						global::UnityEngine.GameObject gameObject = AxiObject.Instantiate(activeGlow, base.transform.position, base.transform.rotation) as global::UnityEngine.GameObject;
						if (Heal_Timer > 1f)
						{
