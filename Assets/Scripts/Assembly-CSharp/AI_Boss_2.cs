...
public class AI_Boss_2 : global::UnityEngine.MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (distance < 50f && (GM.GameOver || GM.onHscene))
		{
			if (GM.Hscene_Num == 0 && GM.Hscene_Timer <= 0f)
			{
				...
				else if (dist_Y < 1f && (global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 1.7f || (isStuck_Front && global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 3.2f)))
				{
	...
	private void Start_Hscene()
	{
		...
		Mon.isInvincible = true;
		GM.mc_Menu.GetComponent<Menu_Control>().H_Object = gameObject;
