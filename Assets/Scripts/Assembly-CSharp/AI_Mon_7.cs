...
public class AI_Mon_7 : global::UnityEngine.MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (distance < 45f && (GM.GameOver || GM.onHscene))
		{
			if (GM.Hscene_Num == 0 && GM.Hscene_Timer <= 0f)
			{
				...
				else if (dist_Y < 1f && (global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 0.3f || (isStuck_Front && global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 2.2f)))
				{
	...
	{
		if (GetComponent<global::UnityEngine.Animator>().GetBool("onAttack") || GetComponent<global::UnityEngine.Animator>().GetBool("onAttackDown") || GetComponent<global::UnityEngine.Animator>().GetBool("onMove") || GetComponent<global::UnityEngine.Animator>().GetBool("onHit"))
		{
			Set_Idle();
		}
	}
	...
	private void Set_Attack()
	{
		...
		if (num > 5 || PC.State == Player_Control.AniState.Sit || PC.State == Player_Control.AniState.Slide)
		{
	...
	{
		if (Attack_Delay < 1.5f)
		{
			Attack_Delay = 1.5f;
		}
	}

	private void Set_DownDelay()
	{
		H_Timer = 0.8f;
	}
	...
	private void Raycasting()
	{
		...
		dist_Y = global::UnityEngine.Mathf.Abs(Player.transform.position.y - (base.transform.position.y - 2.64f));
		if (distance > 45f)
		{
			EnemyState = 0;
		}
		else if (dist_X < 25f)
		{
			...
			else if (on_Chase && global::UnityEngine.Mathf.Abs(base.transform.position.y - Player.transform.position.y) < 12.5f)
			{
				EnemyState = 2;
			}
			else
