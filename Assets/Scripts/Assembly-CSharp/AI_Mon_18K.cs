public class AI_Mon_18K : global::UnityEngine.MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (distance < 45f && (GM.GameOver || GM.onHscene))
		{
		...
		if (GetComponent<global::UnityEngine.Animator>().GetBool("onHit"))
		{
			...
			else if (in_AttackRange && !GM.GameOver)
			{
				Set_Attack();
			}
			PC_Atk_Timer = 3f;
		...
		if (Attack_Timer > 0f)
		{
			...
			if (GetComponent<global::UnityEngine.Animator>().GetBool("onAttack") && Attack_Timer <= 0f)
			{
				Set_Idle();
			}
			return;
		...
		else if (isStuck_Front || dist_X < 5f + rnd_X)
		{
			Check_Idle();
		}
		else
	...
	private void Start_Hscene()
	{
		...
		base.transform.position = new global::UnityEngine.Vector3(Player.transform.position.x + 0.2f * (float)(-facingRight), Player.transform.position.y + 4.03f, 0f);
	...
	private void Raycasting()
	{
		...
		dist_Y = global::UnityEngine.Mathf.Abs(Player.transform.position.y - (base.transform.position.y - 4.03f));
		if (flag || flag2 || flag3 || (dist_X < 15f && dist_Y < 10f))
		{
			in_AttackRange = true;
		}
		else
