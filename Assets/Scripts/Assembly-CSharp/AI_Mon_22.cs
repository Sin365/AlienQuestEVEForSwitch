	...
	private void Update()
	{
		...
		else if (distance < 45f && (GM.GameOver || GM.onHscene))
		{
			if (GM.Hscene_Num == 0 && GM.Hscene_Timer <= 0f)
			{
				...
				dist_H = global::UnityEngine.Vector3.Distance(base.transform.position, new global::UnityEngine.Vector3(GM.gobj_Pos_Down_Center.transform.position.x, Player.transform.position.y + 1.8f, 0f));
				if (dist_H < 1.5f)
				{
					Start_Hscene();
					return;
				}
				Pos_Target = new global::UnityEngine.Vector3(GM.gobj_Pos_Down_Center.transform.position.x, Player.transform.position.y + 1.8f, 0f);
