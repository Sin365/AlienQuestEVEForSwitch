	...
	private void Update()
	{
		...
		else if (GM.GameOver && distance < 45f)
		{
			if (GM.Hscene_Num == 0 && GM.Hscene_Timer <= 0f)
			{
				...
				else if (dist_Y < 1f && (global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 1.3f || (isStuck_Front && global::UnityEngine.Mathf.Abs(base.transform.position.x - GM.gobj_Pos_Down_Center.transform.position.x) < 2.2f)))
				{
