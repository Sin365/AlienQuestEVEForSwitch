...
public class AI_Mon_10 : global::UnityEngine.MonoBehaviour
{
    ...
    private void Start()
	{
		...
		Move_Speed = 5f + global::UnityEngine.Random.Range(0f, 1f);
	...
	private void Update()
	{
		...
		{
			if (DualPlayable && (GM.Hscene_Num == 10 || GM.Hscene_Num == 11))
			{
				if (!GetComponent<global::UnityEngine.Animator>().GetBool("onMove"))
				{
					Set_Move();
	...
	public void Flip()
	{
		...
		CensoredText.transform.localScale = new global::UnityEngine.Vector3(1 * -facingRight, 1.2f, 1f);
		GetComponent<global::UnityEngine.BoxCollider2D>().offset = new global::UnityEngine.Vector2(0.15f * (float)(-facingRight), -0.093f);
	}
	...
	private void Start_H_Dual()
	{
		...
		gameObject.GetComponent<H_Ani>().Mon_Object.transform.position = new global::UnityEngine.Vector3(Player.transform.position.x - 0.8f * (float)GM.GetComponent<H_Control>().facingRight, base.transform.position.y, 0f);
		if ((GM.GetComponent<H_Control>().facingRight < 0 && facingRight < 0) || (GM.GetComponent<H_Control>().facingRight > 0 && facingRight > 0))
		{
			Flip();
		}
		Flip_Delay = 0f;
		base.transform.position = new global::UnityEngine.Vector3(Player.transform.position.x - 2.4f * (float)facingRight, base.transform.position.y, 0f);
	}
	...
	private void Raycasting()
	{
		...
		dist_Y = global::UnityEngine.Mathf.Abs(Player.transform.position.y - (base.transform.position.y - 3.13f));
		if (distance > 45f)
		{
			EnemyState = 0;
		}
