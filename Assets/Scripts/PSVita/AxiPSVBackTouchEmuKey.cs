...
namespace AxiInputSP
{
	...
	public class AxiPSVBackTouchEmuKey : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			{
				m_PSVTouchPosList.Add(new Vector2(50, 50));
			}
#endif

#if UNITY_PSP2 && !UNITY_EDITOR
            for (int i = 0; i < UnityEngine.PSVita.PSVitaInput.touchesSecondary.Length; i++)
            {
                Touch touch = UnityEngine.PSVita.PSVitaInput.touchesSecondary[i];
			...
			foreach (var rangeKV in dictType2Range)
			{
				...
				{
					if (rangeKV.Value.Item1.x < pos.x && pos.x < rangeKV.Value.Item2.x
						&&
						rangeKV.Value.Item1.y < pos.y && pos.y < rangeKV.Value.Item2.y)
					{
						bHad = true;
						break;
					}
